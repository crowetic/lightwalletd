...
func z_gettreestateStub(method string, params []json.RawMessage) (json.RawMessage, error) {
	if method != "z_gettreestate" {
		testT.Fatal("unexpected method in z_gettreestateStub:", method)
	}
	
	// Mock response that matches TreasureChest format
	mockResponse := `{
...
func TestGetTreeState(t *testing.T) {
	testT = t
	common.RawRequest = z_gettreestateStub
	lwd, _ := testsetup()
